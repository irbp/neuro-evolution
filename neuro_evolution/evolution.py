...
class NeuroEvolution:
    ...
    def evolve(self, x_train, y_train, x_val, y_val, x_test, y_test):
        ...
        for i in range(self._generations - 1):
            self._train_networks(x_train, y_train, x_val, y_val, x_test, y_test)
    ...
    def _train_networks(self, x_train, y_train, x_val, y_val, x_test, y_test):
        """
        ...
        for network in self._networks:
            network.train(x_train, y_train, x_val, y_val, x_test, y_test)
            pbar.update(1)
        pbar.close()
